...
class InvoiceBusiness {
  ...
  Create = async (req) => {
    try {
      ...
      const invoiceDetailsObj: IInvoiceDetailCreationAttributes[] = invoiceDetails.map(item => ({
        ...item,
  ...
  GetInvoiceDetailsByInvoiceId = async (req) => {
    ...
    try {
      ...
      const invoice = await this._invoiceRepository.GetInvoiceByCode(invoiceCode);
      if (invoice == null) return BaseBusiness.Error("Invoice do not exist!!");

      const records = await this._invoiceRepository.GetInvoiceDetailsByInvoiceId(invoice.id);
